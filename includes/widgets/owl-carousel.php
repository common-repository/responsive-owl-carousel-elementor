...
class Owl_Carousel extends \Elementor\Widget_Base {
	...
	public function get_style_depends() {
		return array( 'owce-carousel', 'animate', 'owce-custom', 'elementor-icons-fa-solid', 'elementor-icons' );
	}
	...
	protected function render() {
		...
		$layout        = sanitize_text_field( $this->get_owl_settings( 'layout' ) );
		$valid_layouts = array_keys( get_carousel_layouts() );

		if ( ! in_array( $layout, $valid_layouts, true ) ) {
			$layout = 'basic';
		}

		$layout_style = sanitize_text_field( $this->get_owl_settings( 'layout_' . $layout ) ?? 'one' );
		$valid_styles = array_keys( get_carousel_layout_styles( $layout ) );

		if ( ! in_array( $layout_style, $valid_styles, true ) ) {
			$layout_style = 'one';
		}

		$template = OWCE_PLUGIN_PATH . '/includes/widgets/views/' . sanitize_file_name( $layout ) . '/' . sanitize_file_name( $layout_style ) . '.php';
